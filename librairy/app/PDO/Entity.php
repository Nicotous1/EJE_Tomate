<?php
	abstract class Entity implements JsonSerializable 
	{
		...
		public function __construct($params = null, $conv = false)
		{
			...
			if ($conv) {$params = $this->structSQL->convColtoAtt($params);}
		...
		public function set($att, $x) {
			...
			switch ($attSQL->getType()) {
				...
				case AttSQL::TYPE_USER:
					...
					throw new Exception("The attribut '".$att."' is a reference ! It can only be set with a '".$class."', an array or an id !", 1);
...
/*				$method = new ReflectionMethod(get_class($this), $name_method);
		...
		*/	
		...
		public function setId($id) {
			if ($this->id != null && $id != null) {throw new Exception("An entity can not change of id during its lifecycle ! ( ".$this->id." -> ".$id." ) ", 1);} //Incompatible with a cache gestion
			$this->id = (int) $id;
			return $this;
		}
